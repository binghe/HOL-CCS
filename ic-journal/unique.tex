...
\hl{Now we recall the ``unique solution of
contractions'' theorem}~\cite{sangiorgi2017equations},
\hl{which weakens} the requirements of Milner's result
(Theorem~\ref{t:Mil89}).
...
\begin{proof}
\hl{We  prove $P_i \wb Q_i$ (for each $i \in I$) by considering} the following relation
\begin{equation*}
...
\hl{The following two principles were adopted when searching
...
\hl{Furthermore, we needed to show that the definition found is the
best possible one, by showing that it yields the coarsest precongruence
contains in $\mcontrBIS$; the proof is similar
to  the analogous result for $\rapprox$}. \hl{See} Section~\ref{s:coarsest} for
more details.
