...
$\approx$ with $\rapprox$ \DDS{ (note that  with $\rapprox$ one considers
...
in place of  Lemma~\ref{l:uptocon}):}
\begin{equation}
\label{eq:R2}
\R \DSdefi \{ 
(R,S) \st R \wb \ct{\til P}, S \wb \ct{\til Q} \mbox{ for some context
$\qct$} \} \enspace.
\end{equation}
\DDS{The property that $\R$ is a bisimulation  proves  $\til P$ and $\til Q$ bisimilar, i.e.,
  $\til P \wb \til Q$.  To further prove
$\til P \rapprox \til Q$, we  appeal to
Lemma~\ref{l:obsCongrByWeakBisim}. Thus it remains to show that, for 
any two process $P$ and $Q$ and action $\mu$, if $P \arr{\mu} P'$ then
there is $Q'$ such that $Q \Arr{\mu} Q'$  and
$P'\ \R\ Q'$ (and  the converse direction).  The prove for this
part is routine,  using 
Lemma 3.13 of \cite{Mil89},  reported as 
Lemma~\ref{lem:313} in Section~\ref{ss:part2} (the lemma is  also needed 
in  Milner's  proof of ``unique solution of equations for $\sim$'').}
\end{proof}
