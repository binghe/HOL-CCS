...
\hlD{Now we recall the ``unique solution of
contractions'' theorem}~\cite{sangiorgi2017equations},
\hlD{which weakens} the requirements of Milner's result
(Theorem~\ref{t:Mil89}).
...
\begin{proof}
\hlD{We  prove $P_i \wb Q_i$ (for each $i \in I$) by considering} the following relation
\begin{equation*}
\R \DSdefi \{(R,S) \st R \wb \ct{\til P}, S \wb \ct{\til Q} \mbox{~for some context
$\qct$ (\hl{with only guarded sums})} \} \enspace.
\end{equation*}
\finish{below: what is $\equiv$?} 
...
\hlD{See} Fig.~\ref{fig:310} \hl{for a visual illustration.}
\end{proof}
...
in addition to $\wb$).
However, comparing with Milner's Theorem~\ref{t:Mil89s1}, \hlD{there is
...
\hlD{The following two principles were adopted when searching
...
\hlD{Furthermore, we needed to show that the definition found is the
best possible one, by showing that it yields the coarsest precongruence
contains in $\mcontrBIS$; the proof is similar
to  the analogous result for $\rapprox$}. \hlD{See} Section~\ref{s:coarsest} for
more details.
...
\hl{The next lemma is actually Lemma 3.13} of~\citep[p.~102]{Mil89}, \hlD{and
