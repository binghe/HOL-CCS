...
\end{alltt}
In above inductive definitions, for any CCS term $p$,
\HOLinline{\HOLTokenLambda{}\HOLBoundVar{t}.\\\;\HOLFreeVar{p}} is a valid context with no hole.

A context is \emph{weakly guarded} (\HOLinline{\HOLConst{WG}}) if each hole is
underneath a prefix:
\begin{alltt}
...
ordinary
context: $\lambda t. t$ is not weakly guarded as the variable is directly
...
A context is \emph{guarded} (\HOLinline{\HOLConst{SG}}) if each hole is underneath a \emph{visible} prefix:
\begin{alltt}
...
contexts (\HOLinline{\HOLConst{WGS}}) in which only guarded sums are allowed (i.e.~arbitrary sums are forbidden):
\begin{alltt}
...
Although weak bisimilarity ($\approx$) is \emph{not} a congruence
  with respect to~\HOLinline{\HOLConst{CONTEXT}}, it is indeed substitutive
  with respect to~\HOLinline{\HOLConst{GCONTEXT}} 
