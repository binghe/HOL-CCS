...
\hl{In this paper (and the formalisation work), we use the agent
...
\label{ss:SysEq}
\hl{When discussing equations it is standard to talk about `context'. This is a 
...
$E_i$ \hl{possibly containing such variables $\cup_i \{ X_i\}$}. Then 
$\{ X_i = E_i\}_{i\in I}$ is 
...
\item The system has \emph{a unique solution for $\wb$}  if whenever 
 $\til P$ and $\til Q$ are both solutions \hl{then} $\til P \wb \til Q$. 
\end{itemize} 
 \end{definition}
...
\item \hl{\emph{guarded}} if, in each $E_i$, each occurrence of
...
\hl{Milner has  three versions of  ``unique solution of equations''
...
\label{t:Mil89}
Let $E_i$ be \hl{guarded}  and sequential with free
