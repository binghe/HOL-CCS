...
% \emph{bound}.}
A tem
without free variables  is a  \emph{process}.}
...
\hlD{In this paper (and the formalisation work), we use the agent
...
Fig.~\ref{f:LTSCCS}) \hlD{for the substitution of equation variables.
...
\label{ss:SysEq}
\hlD{When discussing equations it is standard to talk about `context'. This is a 
...
$E_i$ \hlD{possibly containing such variables $\cup_i \{ X_i\}$}. Then 
$\{ X_i = E_i\}_{i\in I}$ is 
...
\item The system has \emph{a unique solution for $\wb$}  if whenever 
 $\til P$ and $\til Q$ are both solutions \hlD{then} $\til P \wb \til Q$. 
\end{itemize} 
 \end{definition}
...
\item \hlD{\emph{guarded}} if, in each $E_i$, each occurrence of
  each $X_i$ is underneath a \emph{visible} prefix;

\finish{i have removed 
``(strongly)''} 
...
\hlD{In other words, if a system of equations is sequential, then for
...
\hlD{Milner has  three versions of  ``unique solution of equations''
...
\hlD{The version of Milner's unique-solution theorem for $\wb$ further requires
...
\label{t:Mil89}
Let $E_i$ be \hlD{guarded}  and sequential with free
variables up to $\til X$, and let ${\til P} \wb {\til E}\{\til P /\til X\}$,
  ${\til Q} \wb {\til E}\{\til Q /\til X\}$. Then ${\til P} \wb {\til Q}$.
\end{theorem}

\finish{below and elsewhere: the correct terminology is ``derivative'', instead of ``derivation''} 

The proof of \hlD{the  theorem above} exploits an invariance
property on immediate \hlD{derivatives}
...
\hlD{For more details of above three theorems,} see Section~\ref{ss:part2}
