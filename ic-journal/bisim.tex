...
\begin{alltt}
   \HOLFreeVar{E} \HOLSymConst{\HOLTokenObsCongr} \HOLFreeVar{E\sp{\prime}} \HOLTokenDefEquality{}
...
defined as follows:
\begin{alltt}
   \HOLFreeVar{E} \HOLSymConst{\HOLTokenObsContracts} \HOLFreeVar{E\sp{\prime}} \HOLTokenDefEquality{}
     \HOLSymConst{\HOLTokenForall{}}\HOLBoundVar{u}.
...
privileged relation, so to relieve the proof work needed. This is the so-called
\emph{``up-to'' techniques}, which is a pretty general device that
...
\end{alltt}
\end{definition}
Note that the HOL term of $\sim \R \approx$ is
