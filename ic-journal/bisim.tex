...
\hl{One} highlight of this formalization project is the simplified definitions of
  bisimilarities using the new coinductive relational package of HOL4.
...
what kind of binary relations on CCS process \hl{are (strong) bisimulations}:
\begin{alltt}
...
\begin{lstlisting}
   ...
   ------------------------------------
    ...
    1.  CONTRACTION Con
\end{lstlisting}
We cannot say show that $Con$ itself is a bisimulation, but rather
that the union of $Con$ and $\wb$ is a bisimulation.
\hl{The corresponding lemma for the expansion preorder is rather
straightforward
and does not require such a non-trivial contruction.}
...
\begin{enumerate}
\item Whenever $P \overset{\mu}{\rightarrow} P'$ then, for some
  $Q'$, $Q \overset{\hat{\mu}}{\rightarrow} Q'$ and $P' \sim \S \approx Q'$,
\item Whenever $Q \overset{\mu}{\rightarrow} Q'$ then, for some
  $P'$, $P \overset{\hat{\mu}}{\rightarrow} P'$ and $P' \approx \S \sim Q'$.
\end{enumerate}
...
theorem for $\wb$,
in which we actually used a different bisimulation game defined below:~\cite{sangiorgi1992problem}
