...
\hlD{An} highlight of this formalization project is the simplified definitions of
...
bisimilarity, contraction is not contained in weak bisimulation (while
expansion is),}
i.e. the following proposition does not hold:

\finish{i do not understand above: contraction IMPLIES weak bisimilarity, maybe it is
  harder to prove, i do not know, but the implication is true}
...
\end{alltt}
\hl{For instance, in the proof of \texttt{contracts_IMP_WEAK_EQUIV} (contraction preorder implies $\wb$)}.
We prove it by constructing a bisimulation \HOLinline{\HOLFreeVar{Wbsm}} containing two processes
