...
We have verified the properties that the contraction preorder
($\mcontrBIS$) contains the expansion
...
generally harder than  that for expansion. This is mostly due to the fact
...
privileged relation, so to relieve the proof work needed. These
are usually called \emph{``up-to'' techniques}.
...
\end{alltt}
\end{definition}
The HOL term corresponding to $\sim \R \wb$ is
...
To complete the proof, the following version \emph{with weak arrows} is
actually used:
