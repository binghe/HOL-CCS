...
\label{ss:bb}
\DS{An highlight in this project is the simplified definitions of
...
\end{alltt}
\DS{This is indeed the way followed by Nesi \cite{Nesi:1992ve} at  the
  time of HOL88. With the above definition, deriving
 the following property  \citep[p.~91]{Mil89}  is tedious:}
\begin{quote}
...
\end{alltt}
\end{enumerate}
\DS{The last theorem, \texttt{STRONG_EQUIV_cases}, is indeed  property
(*).}
...
\DS{Following  Nesi \cite{Nesi:1992ve},
...
\DS{Again, we can prove that the identity
relation is a bisimulation, that bisimulation is preserved by inversion,
 composition, and union. With the following call to
...
\end{alltt}
Here is the formal version of Lemma~\ref{l:obsCongrByWeakBisim}, which is needed in the proof
...
\DS{Although weak bisimilarity ($\approx$) is \emph{not} a congruence
...
\DS{On the relationship between bisimilarity and rooted bisimilarity, 
we also prove 
Deng
...
\DS{Having formalised  the definitions of strong bisimulation and strong bisimilarity,
...
previous proven laws, without directly using any SOS
...
\end{alltt}
\DS{As a result, to finish the proof of \texttt{contracts_IMP_WEAK_EQUIV}, we do not prove
...
\DS{``Bisimulation up to'' is a powerful proof technique.}
...
privileged relation, so to relieve the proof work needed. We call
this an \emph{``up-to'' technique}. 
...
\DS{For weak bisimilarity, there are a few versions of the `up-to
...
\DS{
The above version of ``bisimulation up to $\wb$'' cannot be used to prove
Milner's ``unique solution of equations'' theorem for $\wb$}, for
which we need 
the
following version (though less practical to use
as it is  usually harder to
check  multi-step transitions than one-step transitions):}
\begin{alltt}
...
bisimilarity. (It is not however powerful enough to be used   within
