...
theorems (\emph{rules}, \emph{coind} and \emph{cases}):
% , \hl{which
% fully capture weak bisimilarity:}
\begin{itemize}
\item 
% \emph{rules} is a conjunction of implications that will be the
%     same as the input term:
\emph{rules} is the same as the input term, which now becomes a theorem:
...
\hlD{is not recursive and } follows Definition~\ref{d:rootedBisimilarity}:
\begin{alltt}
...
\end{alltt}
\hlD{These are useful results in the theory of CCS (though we will not need them in the
...
% contractions (and of rooted contractions)}, 
We  formalise
 and contraction along
...
Finally, the rooted contraction ($\rcontr$) is formalised 
 as follows:
\begin{alltt}
