...
A highlight of this formalization project is the simplified definitions of
  ...
  how weak bisimulation and weak bisimilarity are defined. 
Strong bisimulation and strong bisimilarity, as well as  other
concepts such as expansion and contraction,  are defined in the same manner.

To define (weak) bisimilarity, we need  weak transitions of processes. 
...
reflexive and transitive closure (RTC, denoted by \mbox{\color{blue}{$^*$}} in
...
largest), but it does not constrain the resulting relation in the set of
...
%
Without HOL coinductive relation package, (weak) bisimilarity
...
For weak bisimilarity and rooted bisimilarity, the following laws, 
called $\tau$-laws, hold:
\begin{alltt}
...
\hl{We have  verified the properties that} the contraction preorder
\hl{($\mcontrBIS$) contains the expansion
preorder ($\expa$), and that they are both}
...
\end{enumerate}
\end{proposition}
The proof of a property for contraction  is
\hl{generally} harder than  that for expansion. \hl{This} is mostly due to the fact
  that, although the contraction preorder ($\mcontrBIS$) is contained in
  bisimilarity ($\wb$), 
\hl{a
  contraction need not be a bisimulation.} In another words,
  the following proposition does not hold: 
\begin{alltt}
...
For instance, 
we can prove 
\texttt{contracts_IMP_WEAK_EQUIV}
...
this definition is sometimes  hard \hl{to apply}. \hl{I}nstead, to reduce
...
% \hl{are general devices allowing} a great variety of possibilities.
...
\hl{``weak bisimulation up to'' relation need not be contained in weak
...
\end{alltt}
\end{definition}
The \hl{HOL term corresponding to} $\sim \R \approx$ is
...
<<<<<<< HEAD
is not powerful \hl{enough} for Milner's ``unique solution of equations''
...
theorem for $\wb$ (see~\cite{sangiorgi1992problem} for more details).
...
\end{enumerate}
\hl{Formally: } % (for showing the correspondence between the informal and formal definitions)}
\begin{alltt}
