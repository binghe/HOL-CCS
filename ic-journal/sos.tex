...
End
\end{lstlisting}
\hl{For instance, if the type $\beta$ is instantiated by the type
...
action $\overline{\text{b}}$ of type ``\HOLinline{\ensuremath{\beta} \HOLTyOp{Action}}''
...
expressions, where \texttt{'a} (or $\alpha$) is the type variable\footnote{The order of type variables $\alpha$ and $\beta$
    is arbitrary. Our choice is aligned with  the \hl{literature.
...
agent variables, is then defined inductively:\footnote{``\HOLinline{\ensuremath{\beta} \HOLTyOp{Relabeling}}'' is the type of all relabeling functions. They (and
...
in the literature, are given in Table~\ref{tab:ccsoperator}, \hl{where
most CCS operators have also the more readable abbreviation forms,
either for end user or for \TeX{} outputs.}\footnote{All
...
\hline
\hlD{nil} & $\textbf{0}$ & \texttt{nil} & \texttt{nil} & \HOLinline{\HOLConst{\ensuremath{\mathbf{0}}}} \\
...
\end{tabular}
\end{center}
   \caption{Syntax of \hl{some CCS concepts in HOL}}
...
\HOLinline{\HOLConst{TRANS}} by HOL's \texttt{HOL_reln} function~\cite{holdesc} for more details.
...
\end{equation*}
\item \hlD{\emph{ind} is the induction principle for the
  relation} (see Section~\ref{sec:multivariate} for the exact form
  and its application).
\item \hl{\emph{cases} is the so-called `cases' or `inversion' theorem for
  ...
  theorem:}
\begin{alltt}
\HOLTokenTurnstile{} \HOLFreeVar{E} \HOLSymConst{\ensuremath{+}} \HOLFreeVar{E\sp{\prime}} \HOLTokenTransBegin\HOLFreeVar{u}\HOLTokenTransEnd \HOLFreeVar{E\sp{\prime\prime}} \HOLSymConst{\HOLTokenEquiv{}} \HOLFreeVar{E} \HOLTokenTransBegin\HOLFreeVar{u}\HOLTokenTransEnd \HOLFreeVar{E\sp{\prime\prime}} \HOLSymConst{\HOLTokenDisj{}} \HOLFreeVar{E\sp{\prime}} \HOLTokenTransBegin\HOLFreeVar{u}\HOLTokenTransEnd \HOLFreeVar{E\sp{\prime\prime}}\hfill{[TRANS_SUM_EQ]}
\end{alltt}
\end{itemize}
...
\end{alltt}
\hlD{The variable substitutions only affects free
variables: the variable $Y$ in} ``\HOLinline{\HOLConst{rec} \HOLFreeVar{Y} \HOLFreeVar{E}}'' \hlD{is
bound and therefore the substitution ignores it. }
% The actual substitution happens in case of}
% ``\HOLinline{\HOLConst{var} \HOLFreeVar{Y}}'' if $Y$ is being substituted (and if
...
following two facts must be proved: \hl{(1) there are indeed the three
  transitions mentioned above; (2) there are no other
  transitions.} For large CCS processes it is surprisingly \hl{tedious} to manually derive
all the possible transitions and prove the non-existence of others.
\hlD{This shows the usefulness of appealing to an ML function
