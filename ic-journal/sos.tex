...
\emph{action} $\overline{b}$ \hl{of type} ``\HOLinline{\HOLTyOp{string} \HOLTyOp{Action}}''
...
other hand, \HOLinline{\HOLConst{label} \ensuremath{(}\HOLConst{name} \HOLNumLit{1}\ensuremath{)}} and \HOLinline{\HOLConst{label} \ensuremath{(}\HOLConst{coname} \HOLNumLit{2}\ensuremath{)}} could be actions \hl{of type} ``\HOLinline{\HOLTyOp{num} \HOLTyOp{Action}}'', where
...
\hl{with respect to} the choice of these two sets.)
%
...
\hl{The} successful invocation of the definitional principle returns three
important theorems \hl{(\texttt{TRANS_rules}, \texttt{TRANS_ind} and \texttt{TRANS_cases})}:
\begin{itemize}
  \item \hl{\texttt{TRANS_rules}} is a conjunction of implications (each
    ...
    extracted from \hl{\texttt{TRANS_rules}}:
\begin{alltt}
...
\end{equation*}
\item \hl{\texttt{TRANS_ind}} is the induction principle for the relation
  (see Section~\ref{sec:multivariate} for its exact statement and an
  application in the proof of Proposition~\ref{prop:transFV}).
\item \hl{\texttt{TRANS_cases}} is the so-called `cases' or `inversion' theorem for
