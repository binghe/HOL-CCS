...
set of names used for labels), respectively}.
%
The type ``\HOLinline{\ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS}}'' \hl{is defined inductively by HOL \texttt{Datatype} package}
(here ``\HOLinline{\ensuremath{\beta} \HOLTyOp{Relabeling}}'' is the type of all relabeling
  functions;   \hl{we have also formalised relabeling, though we will
    not discuss it}):
\begin{lstlisting}
Datatype: CCS = nil
              | var 'a
...
\HOLinline{\HOLConst{TRANS}} by HOL \texttt{HOL_reln} function 
