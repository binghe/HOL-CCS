...
relabeling and the CCS term itself. We use the type ``\HOLinline{\ensuremath{\beta} \HOLTyOp{Label}}''
...
instantiated by the end user \hl{or further formalisations built upon the
present one.}
All \hl{labels} are divided into input and $\overline{\text{output}}$ ones.
...
End
\end{lstlisting}
For instance, if the type $\beta$ is instantiated by \texttt{string}
(the type of all strings), then HOL terms \HOLinline{\HOLConst{name} \HOLStringLit{a}} and
...
expressions, has two type variables \hl{$\alpha$ and $\beta$ corresponding%
\footnote{The order \hl{between} type variables $\alpha$ and $\beta$
...
to $\mathscr{X}$ (the set of agent variables) and $\mathscr{L}$ (the
set of names used for labels), respectively, used at the beginning of} Section~\ref{ss:ccs}.
%
The type ``\HOLinline{\ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS}}'' \hl{is defined inductively by HOL's \texttt{Datatype} package:}
