...
(finite) sets of variables (\hl{of type} \HOLinline{\ensuremath{\alpha}}).
...
A finite map (\hl{of type} \HOLinline{\ensuremath{\alpha} \HOLTokenMapto{} \ensuremath{\beta}}) is like a function
(\hl{of type} \HOLinline{\ensuremath{\alpha} \HOLTokenTransEnd \ensuremath{\beta}}) having only finitely many elements in its
...
map \HOLinline{\HOLFreeVar{fm}} (\hl{of type} \HOLinline{\ensuremath{\alpha} \HOLTokenMapto{} \ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS}}) and a CCS expression,
...
\hl{with respect to}~\texttt{CCS\_Subst}:
...
\hl{of type} ``\HOLinline{\ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS} \HOLTokenTransEnd \ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS}}''. These predicates, such as
...
be used to assert the weak guardedness of $E$ \hl{with respect to} all its free
...
resulting term is still a valid context (\hl{with respect to} the same set of variables):
\begin{alltt}
...
\hl{with respect to} such list of variables:
\begin{alltt}
...
\end{alltt}
On the other hand, for any context (\hl{with respect to} $\til X$) of the form $\recu Y E$,
...
their composability \hl{with respect to}~\multivariate contexts: if we
substitute some free variables in a context $C$ with some weakly guarded
contexts, the resulting context is weakly guarded (\hl{with respect to} the same set of variables):
\begin{alltt}
...
HOL have the same meaning \hl{(they are equivalent)}. The latter form suggests that
...
theorems can be understood thus: any two elements\hl{\ \ }%
