...
\hl{This formalisation  supports finitely many
...
As mentioned at
 the beginning of Section~\ref{s:eq},  
 an
agent variable may  occur outside the recursion in which it is bound;
i.e., a variable can appear both  \emph{bound} and \emph{free} in a term. 
% In Milner's original book chapter~\cite{milner1990operational}, the
...
type ``\HOLinline{\ensuremath{(}\ensuremath{\alpha},
  \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS} \HOLTokenTransEnd
  \ensuremath{\alpha} \HOLTokenTransEnd \HOLTyOp{bool}}'',
i.e. functions taking CCS terms and returning sets of
...
\begin{proof}
We \hl{actually prove} a stronger result,  \hl{asserting} that the set of free
variables \hl{in a CCS process} may decrease through its derivatives:
\begin{alltt}
...
In HOL, the proof 
 ...
 ``action induction''  becomes a form of  higher-order application
of the following \emph{induction principle}, generated together with the SOS
rules (and that essentially says that \HOLinline{\HOLConst{TRANS}} is the smallest relation
...
Taking $E ' = \recu X E$ in the above lemma, we have
    ...
    \HOLinline{\HOLFreeVar{X}\\\;\HOLConst{INSERT}\\\;\HOLConst{BV}\\\;\HOLFreeVar{E}}. This
completes
the    proof.
\end{proof}
...
substitution  $E\{\til P/\til X\}$, that replaces each \hl{free occurrence}
of  variables $X_i$ in $E$ with the corresponding
...
\HOLinline{\HOLFreeVar{Ps}}  by a helper function \HOLinline{\HOLConst{fromList}} (whose details 
are omitted here), while
...
The substitution mechanism is
 order-independent. For most CCS operators it
...
are contained in  the union of the bound variables from $E$ and \HOLinline{\HOLFreeVar{Ps}}.
\begin{alltt}
...
=======
notation) is weakly guarded (cf.~Def.~\ref{def:guardness}).
...
is disjoint from a list of variables, then the expression is
 a context:
=======
...
(cf.~the footnote at the beginning of Section~\ref{ss:SysEq}). In the
...
$E_i$, the set of its bound variables must be \emph{disjoint} from $\til
...
the
set of bound variables %(i.e. process constants) 
...
equations:\footnote{If $R$ is a binary relation of CCS processes,
  ...
  CCS processes. Furthermore, \HOLinline{\HOLConst{LIST_REL}\\\;\HOLFreeVar{R}\\\;\HOLFreeVar{A}\\\;\HOLFreeVar{B}} 
...
\end{alltt}
Note that, in HOL,
\HOLinline{\HOLConst{CCS_solution}\\\;\HOLFreeVar{R}\\\;\HOLFreeVar{Xs}\\\;\HOLFreeVar{Es}\\\;\HOLFreeVar{Ps}\\\;\HOLSymConst{\HOLTokenEquiv{}}\\\;\HOLFreeVar{Ps}\\\;\HOLSymConst{\HOLTokenIn{}}\\\;\HOLConst{CCS_solution}\\\;\HOLFreeVar{R}\\\;\HOLFreeVar{Xs}\\\;\HOLFreeVar{Es}}.
The form in the  right-hand side suggests that
