...
\HOLinline{\HOLFreeVar{Ps}} by a helper function \HOLinline{\HOLConst{fromList}} (\hl{defined by us, and} whose details are
omitted here).
...
\hl{The core idea is that, whenever we replace a variable by a hole and view
the other variables as constants, this is a single variable context
that can be asserted by predicates like} \HOLinline{\HOLConst{CONTEXT}}.
...
their \univariate companions \HOLinline{\HOLConst{SG}} and \HOLinline{\HOLConst{SEQ}}, \hl{but so far
...
w.r.t.}~\HOLinline{\HOLConst{context}}.
...
it is possible that the bound variable $Y$ occurs with another \hl{nested}
recursion, and then it is free in $E$ (as in $\recu Y
(\recu Z (a.Y + b.Z) + c.Y)$).
...
\end{alltt}
Note that, in HOL, the two logical terms ``\HOLinline{\HOLConst{CCS_solution}\\\;\HOLFreeVar{R}\\\;\HOLFreeVar{Xs}\\\;\HOLFreeVar{Es}\\\;\HOLFreeVar{Ps}}'' and ``\HOLinline{\HOLFreeVar{Ps}\\\;\HOLSymConst{\HOLTokenIn{}}\\\;\HOLConst{CCS_solution}\\\;\HOLFreeVar{R}\\\;\HOLFreeVar{Xs}\\\;\HOLFreeVar{Es}}'' have the same meaning
(w.r.t.~\texttt{pred_set} theory). The latter form suggests that,
...
theorems can be understood in this way: any two (syntactically different) elements
