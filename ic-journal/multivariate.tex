...
\hl{In this section we attack the case of 
...
We chose these two theorems because \hlD{of their relevance}, and 
because they well illustrate the \hlD{work} needed in the \multivariate case.
...
\hl{applications of} $\lambda$-functions to CCS terms.
This idea, however,
...
new primitive (e.g.~\texttt{Var X}) to the existing CCS type (besides
 agent variables such as \texttt{var X}).
...
For instance, the variable substitution \hlD{in the  SOS rule}
...
difficulities in \hl{transition inductions}. As an evidence,
...
variables in a CCS process \hlD{does not increase} in its derivatives:
\begin{alltt}
...
\hlD{As a matter of fact, in the  HOL proof}
 ``action induction'' %(or \emph{transition induction}) 
...
proposition (\hl{taking} $E$, $\mu$ and $E'$), and $X$ is another higher-order
proposition.
\hl{The proof goal} is actually in the form of
...
There are two natural ways to \hlD{implement} % not "represent" but actually "do"
...
called \HOLinline{\HOLConst{CCS_SUBST}}, based \hl{on} HOL
...
all occurrences of variables in \hl{the finite domain of} \HOLinline{\HOLFreeVar{fm}} are substituted with
the corresponding value in \HOLinline{\HOLFreeVar{fm}}. \hl{Initially,} such a finite map can be built from
...
Using maps, the substitution mechanism is order-independent. For most CCS operators,
\HOLinline{\HOLConst{CCS_SUBST}}
 recursively calls itself on subterms.  \hlD{The} most
...
(\hl{multi-hole} contexts), \HOLinline{\HOLConst{WG}} (weak guarded contexts),
...
in a bottom-up \hl{manner}. For instance, \HOLinline{\HOLConst{WG}\\\;\ensuremath{(}\HOLTokenLambda{}\HOLBoundVar{t}.\\\;\HOLFreeVar{a}\HOLSymConst{\ensuremath{\ldotp}}\HOLBoundVar{t}\\\;\HOLSymConst{\ensuremath{\mid}}\\\;\HOLFreeVar{P}\ensuremath{)}}
...
guarded variants, but care is needed
for 
 variables that occur \hl{within a  recusion}.  \hl{We have preferred a different
   solution,} in which the definitions of 
 \multivariate contexts  is based on the
 existing \univariate definitions.
\hlD{For this, intuitively,  we replace a single variable with a hole  (viewing
...
we focus on each (free) variable, we substitute each
occurrence of a variable, say $X$, with a ``hole'' and see the
...
preorder ($\mcontrBIS$), \hl{as they are not (pre)congruence relations}.

Another important property of contexts is their composability: if we
substitute some free variables in a context \hl{with} some other contexts, the
...
%, as a bound
%variable of $\recu Y E$, may also be a free variable in $\til X$.
 We
also cannot conclude \HOLinline{\HOLConst{context}\\\;\HOLFreeVar{Xs}\\\;\HOLFreeVar{E}}, because in $\recu Y E$
the bound variable $Y$ \hlD{may occur {freely} within another nested}
recursion. \hl{For instance,} variable $Y$ is free   
in $\recu Y E $ , for 
 $E = \recu Z (a.Y + b.Z) + c.Y$.

For weakly guarded contexts, % (which are also normal contexts), 
besides
their usual properties  as in the \univariate case (i.e., weak
guardedness,
\HOLinline{\HOLConst{WG}}), we also need
...
contexts, the resulting \hlD{context is} weakly guarded (w.r.t.~the same set of variables):
\begin{alltt}
...
\end{itemize}
The two lists $\til X$ and $\til E$ must have the same
length, and  the variables in $\til X$ should  be distinct.
Furthermore,  $\til E$ does not contain
 free variables that are not in $\til X$.
\hlD{Finally, } for each
$E_i$, the set of its bound variables must be \emph{disjoint} from
$\til X$. 
% \hl{(In practice, however, one can always
...
\hlD{equations.} Then,  ${\til P} \ \R\  {\til E}\{\til
P/\til X\}$ must hold:
\begin{equation*}
  \begin{cases}
    &P_1 \ \R\ E_1\{\til P/\til X\} \\
...
equations:\footnote{If $R$ is a binary relation of CCS processes,
 ...
 % Furthermore,  \HOLinline{\HOLConst{LIST_REL}\\\;\HOLFreeVar{R}\\\;\HOLFreeVar{A}\\\;\HOLFreeVar{B}}
...
\hlD{Now} the \multivariate version of Theorem~\ref{t:rcontraBisimulationU}:
\begin{alltt}
\HOLTokenTurnstile{} \HOLConst{CCS_equation} \HOLFreeVar{Xs} \HOLFreeVar{Es} \HOLSymConst{\HOLTokenConj{}} \HOLConst{weakly_guarded} \HOLFreeVar{Xs} \HOLFreeVar{Es} \HOLSymConst{\HOLTokenConj{}}
...
various proof \hl{steps} can even be copied from the \univariate \hl{case}.
