...
\paragraph{Sstrong bisimularity}
\DS{Based on results about ``bisimulation up to $\sim$'', we have first proved the following
...
\DS{Suppose the expression $E$ contains at most the variable $X$, and let $X$ be weakly
guarded in $E$.}
\begin{equation}
...
\end{alltt}
\end{theorem}
\DS{In above proof, we have identified 14 major sub-goals, dividing them into 7
...
\DS{Milner's book  \cite{CITATION???}
contains only two ``unique solutions of
...
\DS{We can then derive the target theorem.}
...
\DS{For the ``unique solutions of
...
%  Theorem~\ref{t:rcontraBisimulationU} \hl{can also be seen as a more
