...
\hl{Using ``bisimulation up to $\sim$'' technique}, we obtain the following
...
\end{alltt}
\hlD{Using Lemma}~\ref{l:obsCongrByWeakBisim}, \hl{the next two results} are proved by 
...
on the syntax of the equation, \hlD{using only guarded sums.}
The related formal proofs are tedious but closely follow the informal
proof~\citep[p.~158--159]{Mil89}, \hlD{with the exception of the use  of} ``bisimulation up
...
\hlD{In particular, we use} induction on the length of weak transitions.
...
\emph{traces}, \hlD {sequences of transitions.}
...
\hl{Then,} given any labeled transition relation \HOLinline{\HOLFreeVar{R}} (of  type
...
a relation representing \hl{traces over} \HOLinline{\HOLFreeVar{R}}  (of type
...
\hlD{In a  trace  $P\overset{acts}{\longrightarrow}Q$,}
...
\hlD{If there is at most one visible action $\mu$  in $acts$, and all other actions are $\tau$'s,}
...
label'' for a list of  actions is as follows, where \texttt{MEM} tests if \hl{an} element is a member of a list:
\begin{alltt}
...
label in the rest of list (here $\HOLTokenDoublePlus$ \hlD{is the concatenation
of lists}):
\begin{alltt}
...
\hlD{Finally, the  relationship} between traces and weak transitions is stated
and proved in the following theorem.  \hl{It} says that a weak transition $P \Arr{u} P'$ is also a
...
the main property needed from \hlD{contraction and rooted contraction is} precongruence.
...
\texttt{WG} rather than \texttt{WGS}).
% (cf.~\texttt{UNIQUE_SOLUTION_OF_CONTRACTIONS}).

Having removed the constraints on \hl{summation}, 
