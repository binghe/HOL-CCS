...
\paragraph{For strong bisimularity}
\hl{Based on results of ``bisimulation up to $\sim$'', we have first proved the following
non-trivial lemma. It states in effect that if $X$ is weakly guarded
...
\end{alltt}
\end{theorem}
In above proof, we have identified 14 major sub-goals, dividing into 7
groups, in which each pairs are symmetric (thus having similar proof
steps). The proof of this last theorem consists of 500 lines (each
line usually have 2 or 3 HOL tactics, to make the proof not too long
in lines).

\paragraph{For weak bisimilarity}
Actually Milner's book contains only two ``unique solutions of
equations'' theorems, one for strong equivalence, the other for
observational congruence (with a wrong proof).   But there's indeed a
version for weak equivalence which shares a large portion of proof
steps with the case for ``observation congruence''.  The problem is, since weak
equivalence is not congruence, to make correct statement of this
...
requires more restrictions on the expression, namely, strong guardness.
Now we're ready to state and prove the following lemma:
\begin{lemma}
If the variable $X$ are (strongly) guarded and sequential in $G$, and
$G\{P/X\}\overset{\alpha}{\rightarrow} P'$, then $P'$ takes the form
$H\{P/X\}$ (for some expression $H$), and for any $Q$,
...
\paragraph{For rooted bisimilarity}
For ``unique solutions of
equations'' theorem of rooted bisimilarity, we have used a whole new technique, i.e. the
following lemma:
\begin{lemma}
To prove two processes $E$ and $E'$ are observational congruence, it's
enough to construct a bisimulation with additional observational
...
This lemma can be easily proved by definition of observational
congruence and weak equivalence, however it's never mentioned in
Milner's book. Actually we think it's impossible to prove the
corresponding ``unique solutions'' theorem without this result.
What we have proved is the following one. Notice that, we didn't use
any ``bisimulation up-to'' techniques, because they're not applicable
once above theorem \texttt{OBS_CONGR_BY_WEAK_BISIM} is used. Instead
we have directly constructed a bisimulation to finish the proof.
\begin{lemma}
If the variable $X$ are (strongly) guarded and sequential in $G$, and
$G\{P/X\}\overset{\alpha}{\rightarrow} P'$, then $P'$ takes the form
$H\{P/X\}$ (for some expression $H$), and for any $Q$,
...
has more severe constraints (must be both strongly guarded and sequential):
The Theorem~\ref{t:rcontraBisimulationU} \hl{can also be seen as a more
