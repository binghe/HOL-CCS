...
As a consequence, theories and proofs verified in HOL are easier to understand
 \hl{for} people who are not familar with more advanced dependent type
 theories, \hl{e.g. the Calculus of Constructions implemented in Coq.}
...
\hl{HOL terms} represent elements of the
sets denoted by \hl{their} types. There are four kinds of \hl{HOL terms,
which} can be described (in simplified forms) by the BNF grammar in
Fig.~\ref{fig:hol-terms}. (See~\cite{hollogic} for a complete
description of HOL, \hl{including the primitive derivative rules to be
mentioned below.})
