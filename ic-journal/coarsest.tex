...
The formal proofs of Theorems~\ref{thm:coarsestR} and % editorial
...
This can be done by following Klop's construction. % editorial
...
they are all pairwise non-bisimilar, % editorial changes
...
in $\mathscr{L}$.
This property is captured by appealing to the following set-theoretic % editorial changes
lemma (see~\cite{Tian:2017wrba} for its proof): % editorial changes
\begin{lemma}
...
(\texttt{COARSEST_CONGR_FINITE}). The same proof idea can also be used with
 contraction and rooted contraction. % editorial changes
