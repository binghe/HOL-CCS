...
with our CCS syntax (\hl{which has} only binary sums)
we have to add an hypothesis about
 the involved processes not using all available labels.
...
$\rapprox$ for summation and the fact that $\rapprox$ implies $\wb$:
\begin{alltt}
...
and $q$ do not use all available labels (i.e.~visible actions).
Formalising this result, however, requires a detailed treatment of free and bound names of CCS
processes (with the restriction operator acting as a binder).
In our formalisation, in contrast,
we assume \hl{a} weaker condition that all immediate \emph{weak} derivatives of
 $p$ and $q$ do not use all available labels. We call this
...
(hence the composition closure is the coarsest one):
\begin{alltt}
...
% \begin{alltt}
% \HOLConst{WEAK_CONGR} \HOLTokenDefEquality{} \HOLConst{CC} \HOLConst{WEAK_EQUIV}
% \end{alltt}
...
% \begin{alltt}
% \HOLConst{SUM_EQUIV} \HOLTokenDefEquality{} \ensuremath{(}\HOLTokenLambda{}\HOLBoundVar{p} \HOLBoundVar{q}. \HOLSymConst{\HOLTokenForall{}}\HOLBoundVar{r}. \HOLBoundVar{p} \HOLSymConst{\ensuremath{+}} \HOLBoundVar{r} \HOLSymConst{\HOLTokenWeakEQ} \HOLBoundVar{q} \HOLSymConst{\ensuremath{+}} \HOLBoundVar{r}\ensuremath{)}
% \end{alltt}
...
The formalisation of (\ref{equa:pq}) says:
