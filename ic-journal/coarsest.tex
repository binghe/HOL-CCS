...
\hl{Since the type} ``\HOLinline{\ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS}}'' is \hl{parameterized with two
...
To prove Theorem~\ref{thm:coarsestfiniteState}, \hl{it only remains to construct this stable
...
\end{alltt}
\end{lemma}
\hl{Then, }combining the above lemma with \texttt{PROP3_COMMON} and \texttt{COARSEST_CONGR_RL},
\hl{we finally proved Theorem}~\ref{thm:coarsestfiniteState}
(\texttt{COARSEST_CONGR_FINITE}). \hl{(In the future, we seek a method to
