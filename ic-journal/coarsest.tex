...
to add \hlD{an} hypothesis \hl{that}
 the involved processes \hlD{do not use} all available labels.
...
we assume \hlD{the} weaker hypothesis that all \emph{immediate weak} derivatives of
$p$ and $q$ do not \hlD{use} all available labels.
We \hlD{call this} the \emph{free action} property:
\begin{alltt}
...
\hlD{As the type} ``\HOLinline{\ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS}}'' is \hl{parameterized on two
...
Proposition~\ref{prop:coarsest};  \hlD{however,  due to some
...
(i.e.~\hl{without  $\tau$ transitions})
 process which is not bisimilar with any \hl{weak} derivative of $p$ and
...
To prove Theorem~\ref{thm:coarsestfiniteState}, \hl{it only remains to construct such stable
...
any \hlD{weak derivative} of the two root processes.
...
For any ``\HOLinline{\HOLConst{label}\;\HOLFreeVar{a}}'', having a function ``\HOLinline{\HOLConst{KLOP}\;\HOLFreeVar{a}}'' (of  type ``\HOLinline{\HOLTyOp{num} \HOLTokenTransEnd \ensuremath{(}\ensuremath{\alpha}, \ensuremath{\beta}\ensuremath{)} \HOLTyOp{CCS}}'')
\hlD{defined on the natural numbers, we obtain a countable set of processes
...
({even}
when $a$ is the only element of type $\beta$, i.e. the only label name
in $\mathscr{L}$).
...
\begin{proposition}
If $p$ has a weak transition to $q$, then $q$ is among \hlD{the derivatives} of $p$:
\begin{alltt}
...
Using all the above results, now we can prove the following \hl{finite-state}
