...
\hlD{\emph{Strong} bisimulation and strong bisimilarity} ($\sim$)
...
guarded sums), i.e.\  $P_1 \wb Q_1$ and $P_2 \wb Q_2$  \hlD{does not imply}
 $P_1 + P_2 \wb Q_1 + Q_2$.
...
\hlD{The above definition also brings up a proof technique for proving rooted
...
\hlD{Given a (weak) bisimulation $\RR$, suppose two processes $P$ and $Q$
...
\hlD{One basic property of rooted bisimiarity is congruence,
indeed
 the
coarsest congruence contained in bisimilarity}
(c.f., Section~\ref{s:coarsest} \hlD{for more details): }
\begin{theorem}
