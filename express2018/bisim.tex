...
In HOL4 Kananaskis-11 and later releases, the new \texttt{Hol_coreln} command accepts a conjuction of relation rules and build a coinduction relation, whose property is fully captured by three theorems as outputs. Here is what we got from HOL using above single rule in Def.\;{d:wb2}:
\begin{enumerate}
...
\end{alltt}
\end{small}
\item The coinductive principle for $\wb$: (it reads as \begin{small}``for any relation $\mathrm{WEAK\_EQUIV'}, \ldots$''\end{small})
\begin{small}
\begin{alltt}
...
The coinductive principle \texttt{[WEAK_EQUIV_coind]} never appears in any CCS textbook, the fixpoint theorem \texttt{[WEAK_EQUIV_cases]} ``property star (*)'' by Robin Milner\cite{Mil89}. It's very useful in proving
many results regarding bisimilarity. It's very hard\footnote{In the original CCS formalization in HOL88, Monica Nesi has to follow this hard way, the original proof (for strong bisimilarity) appears at p. 91 of \cite{Mil89}, the formal proof by Monica Nesi now lives in \texttt{ExampleScript.sml} for historical references.} to directly prove this theorem from Def.\;\ref{d:wb1}.
