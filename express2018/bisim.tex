...
The coinductive principle \texttt{[WEAK_EQUIV_coind]} never appears in any CCS textbook, the fixpoint theorem \texttt{[WEAK_EQUIV_cases]} ``property star (*)'' by Robin Milner. It's very useful in proving
many results regarding bisimilarity, while it's quite hard\footnote{In the original CCS formalization in HOL88, Monica Nesi has to follow this hard way, the original proof (for strong bisimilarity) appears at p. 91 of \cite{Mil89}, the formal proof by Monica Nesi now lives in \texttt{ExampleScript.sml} for historical references.} to directly prove this theorem from Def.\;\ref{d:wb1}.
