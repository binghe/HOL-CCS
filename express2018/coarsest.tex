...
on CCS processes, {no matter if it is (pre)congruence or not},
the context closure of $R$ is always finer}\footnote{We 
need not put $R\ g\ h$ in the antecedent in
\texttt{CC\_def}, as this  is anyhow obtained 
from the trivial context $\lambda x.\,x$.} than
...
\end{alltt}
\hl{Furthermore, we  prove that any (pre)congruence contained in $R$
%  (which
% itself may not be) 
is contained in the context
closure of $R$; hence such context
closure is the coarsest (pre)congruence:}
\begin{alltt}
...
The above proof precisely follows \cite{Mil89}. \hl{(If only $p$ (or $q$) has the free action while the other has used  all available
labels, the classic assumption $\mathrm{fn}(p) \cup
\mathrm{fn}(q) \neq \mathscr{L}$, where $\mathrm{fn}$ stands for \emph{free
  names}, does not hold, and the proof cannot be completed.)} Our result is a bit stronger in the
sense that, $p$ and $q$ do not really need to have the \emph{same} free action.

\hl{There exists a  different,  more complex, proof,
