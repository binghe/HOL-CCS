...
\begin{equation}
\label{eq:coarsest}
\forall p\ \ q.\ p\ \approx^c\ \! q\ \Leftrightarrow\ ( \forall r.\ p\ +\
...
\Rightarrow\ p\ \approx^c\ \! q  \enspace.
\end{equation}
...
steps are almost the same, and the corresponding HOL theorem says:
...
if only $p$ (or $q$) has free actions while the other uses up all available
...
\emph{same} free action. ($a$ and $\outC a$ are \emph{different} actions.)
...
any two processes $p$ and $q$, if there exists a \emph{stable} (i.e. $\tau$-free)
 ...
 $q$, then \texttt{SUM_EQUIV} indeed implies rooted bisimilarity ($\approx^c$):
\begin{alltt}
