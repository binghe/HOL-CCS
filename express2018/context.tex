...
contexts (\texttt{WGS}) in which only guarded sums are allowed (i.e. direct sums are forbidden):}
\begin{alltt}
...
\texttt{CONTEXT}. \hl{The only difference between congruence and
precongruence is, the former must be an equivalence (reflexive,
...
\hl{For example, we can prove that, strong bisimilarity ($\sim$) and
...
\end{alltt}
\hl{Weak bisimilarity ($\approx$) is \emph{not} congruence
  w.r.t. \texttt{CONTEXT}. However, it is ``congruence''
  w.r.t. \texttt{GCONTEXT} (or if the CCS syntax were defined with
  only guarded sum operator, as in \cite{sangiorgi2015equations}) because $\approx$ is indeed preserved by 
  weakly-guarded sums.}
