...
\begin{itemize}
\item
\textbf{weakly guarded} if,  in each    $E_i$, each occurrence of
an \behav\  variable is underneath a prefix;

\item
\textbf{(strongly) guarded} if,  in each    $E_i$, each occurrence of
an \behav\  variable is underneath a \textbf{visible} prefix;

\item 
 \textbf{sequential} if,  in each    $E_i$, each of its subexpressions with occurrence of
