...
For this, we first 
 define \hl{the concept of list-accumulating reflexive transitive closure
 (\texttt{LRTC})}.
Given a labelled transition relation \texttt{R}, \texttt{LRTC R} is
a 3-ary relation representing the trace of transitions:
\begin{alltt}
...
\hl{The trace relation for CCS can be then obtained
 by composing \texttt{LRTC} with the (strong) transition
relation \texttt{TRANS}}:
\begin{alltt}
\HOLConst{TRACE} \HOLSymConst{=} \HOLConst{LRTC} \HOLConst{TRANS}\hfill{[TRACE_def]}
\end{alltt}

\hl{Notice that if the list of actions is empty, that means that there is no transition and hence,}
if there is at most one visible action (i.e. a label) in the list of actions,
then the trace is also a weak transition. Here
we must distinguish between two cases: no label and unique label (in
...
following definition (learnt from Robert Beers) has
avoided any counting or filtering in the list.}
...
to see that, a weak transition either gets shorter
or remains the same when passing into $\mcontrBIS$ from left to right}.
\hl{This fact, among others, is crucial} in proving
...
Finally, \hl{with all above work carefully done with patience,}
 we have formally proved Theorem~\ref{t:contraBisimulationU}:
\begin{alltt}
