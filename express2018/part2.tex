...
Now we show the formalized statements of Lemma~\ref{l:uptocon}, which
is quite long:\vspace{-1em}
\begin{alltt}
\begin{small}
...
\end{small}
\end{alltt}
\vspace{-1em} Traces were involved in the proof of above lemma by using
...
\end{small}
\end{alltt}
It roughly says, for any context $C$ and weakly guarded expression
$E$, if $C \circ E^n[P] \overset{xs}{\Longrightarrow} P'$ and the length
of actions $xs \leqslant n$, then $P'$ has the form of $C'[P]$, meaning
$P$ is not touched during the transitions. Without using traces it is
impossible to reason on the length of intermediate actions behind weak transitions.
...
(Having proved
the precongruence property for the rooted contraction, 
we can use the
